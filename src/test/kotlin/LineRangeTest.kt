...
internal class LineRangeTest {
    ...
    /** Infinite ranges can't be tested for equality, so compare string representation. */
    ...
    fun single() =
        listOf(
            ...
            .map { (input, expected) -> DynamicTest.dynamicTest(input, input shouldBe expected) }
    ...
    fun multiple() =
        listOf(
                ...
                "5-20,23-56,58-61,63-64,69-76" to
                    (5..20) + (23..56) + (58..61) + listOf(63, 64) + (69..76),
            )
            .map { (input, expected) -> DynamicTest.dynamicTest(input, input shouldBe expected) }
}
