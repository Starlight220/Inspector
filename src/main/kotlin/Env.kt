...
inline fun <T> EnvSet.asContext(action: context(EnvContext) () -> T) = with(EnvContext(this), action)

/** Context struct for the entire run. */
@Serializable
data class EnvSet(
    val sources: List<RliSet>,
    ...
    companion object {
        fun fromInputs(): EnvSet {
            ...
            val baseUrl = getInputOrThrow("baseUrl").let { str -> if (str.endsWith('/')) str else "$str/" }
            val latestVersion = getInputOrThrow("latestVersion")
            val versionScheme = getInputOrThrow("versionScheme")

            return EnvSet(sources = listOf(RliSet(baseUrl, versionScheme, latestVersion)), ignoredFiles)
        }
    }
